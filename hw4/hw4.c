...
int main( int argc, char **argv )
{
	...
	int				i, j, k, threshold[3], max[3], x, y, s, xtemp[ROWS][COLUMNS], ytemp[ROWS][COLUMNS], temp[ROWS][COLUMNS];
	FILE			*fp;
	...
	for ( k = 0; k < 3; k ++)
	{
		...
		for( i = 1; i< ROWS-1; i++){
			for( j = 1; j< COLUMNS-1; j++){
				xtemp[i][j] = abs(image[i-1][j-1] + 2*(image[i][j-1]) + image[i+1][j-1] - image[i][j+1] - 2*(image[i+1][j+1]) - image[i-1][j+1]);
				if (xtemp[i][j] > max[0]){
					max[0] = xtemp[i][j];
				}
				ytemp[i][j] = abs(image[i+1][j-1] + 2*(image[i+1][j]) + image[i+1][j+1] - image[i-1][j-1] - 2*(image[i-1][j]) - image[i-1][j+1]);
				if (ytemp[i][j] > max[1]){
					max[1] = ytemp[i][j];
				}
				temp[i][j] = (xtemp[i][j]*xtemp[i][j]) + (ytemp[i][j] * ytemp[i][j]);
				if (temp[i][j] > max[2]){
					max[2] = temp[i][j];
				}
		...
		for( i = 0; i< ROWS; i++){
			for( j = 0; j< COLUMNS; j++){
				if (i == 0 || i == ROWS-1 || j == 0 || j == COLUMNS-1){
					...
					ximage[i][j] = ((double) xtemp[i][j])/max[0] * 255;
					yimage[i][j] = ((double) ytemp[i][j])/max[1] * 255;
					simage[i][j] = ((double) temp[i][j])/max[2] * 255;
			}
		}
